...
class ActorProb(nn.Module):
    ...
    def __init__(
        ...
        self._max = torch.tensor(max_action).to(self.device)
...
class ActorProbBeta(nn.Module):
    ...
    def __init__(
        ...
        input_dim = getattr(preprocess_net, "output_dim", preprocess_net_output_dim)
        self.alpha_head = nn.Sequential(nn.Linear(
            input_dim,  # type: ignore
            self.output_dim,
            device=self.device), nn.Softplus()
        )
        self.beta_head = nn.Sequential(nn.Linear(
            input_dim,  # type: ignore
            self.output_dim,
            device=self.device), nn.Softplus()
        )
    ...
    def forward(
        ...
        """Mapping: obs -> logits -> (mu, sigma)."""
        logits, hidden = self.preprocess(obs, state)
        alpha = (self.alpha_head(logits)) + 1
        beta = (self.beta_head(logits)) + 1
