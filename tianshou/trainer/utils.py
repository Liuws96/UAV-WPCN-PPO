...
def test_episode(
    ...
    if logger and global_step is not None:
        infos = dict(bu.info)
        train_infos = {"rew": bu.rew.mean()}
        for key in infos.keys():
            if key.endswith("sum"):
                train_infos.update({f"test/{key}": np.sum(infos[key])})
            elif key.endswith("mean"):
                train_infos.update({f"test/{key}": np.mean(infos[key])})
        train_infos.update({"len": result["len"], "n/ep": result["n/ep"], "rew_std": result["rew_std"], "len_std": result["len_std"]})
        logger.log_test_data(train_infos, global_step)
